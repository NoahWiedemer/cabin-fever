...
- **Infected**
  ...
  - **Worker**: a Mauler in overalls and a yellow hard hat, from round 2 (about a fifth of the plain infected). It fights like a Mauler but is a little slower, and its helmet makes 45 % of headshots glance off with a spark and a ping (the round is spent, the head snaps back, no damage).
  - **Survivalist**: an infected prepper with a huge backpack, rare (in about 40 % of the rounds from round 2, 55 % from round 8, one at most). It's a bit tougher (190 HP) and pays $150. Killed, it drops its stash: 45 % of the time a good primary you don't carry (P90, R-201, SPAS-12, Devotion, SIGMA or Softball; E picks it up), otherwise a gold-trimmed **survival pack** (see Supply drops).
  - **Boomer**: bloated. When it reaches you it swells up, its skin going livid and throbbing, dribbles bile and bursts. It also bursts when it dies, and shooting its gut sets it off. There's no fire: it pops like a sack of rotten flesh, throwing chunks of meat that bleed in the air, slap onto the walls and lie about for a while, and splattering the floor and the walls around with blood and bile.
  ...
  - **Crusher**: a huge blue tank, from round 11, the boss of a wave. Its death leaves a blue acid cloud.
    - It's armoured: 2,800 HP on hard before the round scaling (10 % more per fireteam member past the first), headshots count only ×1.5 (its skull is armoured; everyone else takes ×4) and explosions hurt it less (×0.6).
    - Its overhead slam hits the ground: everyone within 2.8 m of where the fists land takes the blow (less at the edge) and is thrown back, and the floor shakes.
    - With a target 5 to 15 m away along a clear line (no barricade on it) it roars, stamps and lowers its head for 0.8 s, then charges at 8.5 m/s for up to 15 m. Anyone in its way takes 42 (on hard) and is thrown aside. It only steers a little, so a sidestep dodges it. If it runs into a wall (or a barricade, which it smashes), it's dazed for 1.3 s and takes 40 % more damage.
    - Below 40 % HP it rages: a roar, its skin glows blue, it moves 30 % faster, slams quicker and charges more often.
    - Kill it with rifles at the head in about 6 s of solid fire alone (body shots: about 11 s). It pays $600.
  - **Stalker**: a gaunt, pale mutant, taller than you. It haunts you in every round from round 3 (4 on easy), and on into the buy phase after it. It isn't part of the wave: it doesn't hold up the end of a round, the INFECTED count and the radar leave it out, and it only ever goes for you (the bots don't see it until it attacks).
    - It knows every way into and through the farmhouse and the barn (only the hayloft ladder is out of its reach). It appears and vanishes only where you can't see it: behind a wall, round a corner, behind a boarded-up window or outside your view.
    - It shows up 6 to 15 s into a round, then every 14 to 30 s (a little less often in the buy phase, and never to attack there). Half the time it **watches**: mostly it steps out of cover (a doorway, a corner, a tree) into your view and stands there, preferably under a lamp; else it waits where you'll see it once you turn round. It stares and breathes. Once you've seen it, it holds your gaze for a few seconds, then backs out of sight or bolts; look away first and it's gone. Or it **dashes** across your view at 9 to 11 m/s, out of cover and into cover again. Or, with you in the farmhouse or near it, it **runs through the house**: in through one door with a bang (sometimes a shriek), past you, out through another, its bare feet booming on the floorboards.
    - After three scares it **attacks** (never twice within 70 s): it bursts out of cover in front of you, screams and charges, and lunges at your face. Either it holds you for a second (you can't move, your view is dragged onto its face, you can shoot) and claws you, or it slashes you in passing. Then it runs. It comes at you from the front and circles round if you turn away. It holds you at most once per attack, so there's no stun lock.
    - It can only be hurt while it attacks (and for a moment after). Then 220 HP on hard (a few more each round) go quickly: about 7 rifle hits, 2 headshots. Shot at any other time, it flinches, hisses and runs off. Killed, it drops a red supply box and stays away for the rest of that round and the next.
    - `__game.stalker.force('watch' | 'peek' | 'behind' | 'dash' | 'house' | 'attack')` brings it now, for testing (in a fight round).
- **How the horde hunts**
...
- **Supply drops**
  ...
  - Gold-trimmed olive box (the Survivalist's survival pack, stays 60 s): full HP, AP up to 100, full ammo, and one more frag, Molotov and barricade kit each (up to the carry caps).
...
- `src/nav`
  - `navgrid.js`: a multi-level grid with stair portals (and ladders via `addPortal`). Its costs are 8-connected with no corner cutting, plus a clearance cost next to obstacles. Flow fields label each cell with the survivor they lead to, and `compute()` can stop at a distance for a cheap local field. `steer()` string-pulls to the farthest cell in a clear walkable line. `lineClear()` tests a walkable straight line, and `refreshRect()` re-rasterizes after the level changes.
...
- `src/actors`
  ...
  - `gltfCharacter.js`: rigged GLB bodies (L4D2 Coach and Ellis, the Scorpion soldier, Viper, the UniRig zombie in the Smoker slot, the zombie woman, the gas-mask zombie and the plain zombie among the Maulers, the Worker, the Survivalist, the Meshy "Fat Zombie" Boomer, the Biter kid, the Stalker and the mutant dog). A proxy skeleton with `rig.js` bone names is retargeted onto the model's joints, so one set of procedural animation drives both kinds of body. Valve, Mixamo and Meshy joints are matched by name (Meshy's spine counts down from the neck: `Spine02` is the lowest). Other skeletons, such as UniRig's `Bone_NNN`, are matched by hierarchy and joint position. Hit volumes are measured from the skinned mesh. If a GLB is missing, the procedural body is used instead. A body with `glow` gets its own materials, so its skin can change per instance (the Crusher's rage, the Boomer's swell).
  - `zombie.js`: the Infected AI, animation and hit volumes. The chase covers target choice, direct pursuit with an intercept, entrance routes, stairs, stuck recovery, wall and crowd avoidance, and doorway queueing (see the header). Types are data (`ZOMBIE_TYPES`): besides speed, damage and size, `helmet` (the Worker's chance to deflect a headshot, read by `game.hitscan`) and `loot` (the Survivalist's stash, `game._dropLoot`).
  ...
  - `crusher.js`: the Crusher, a `Zombie` subclass (`CRUSHER` tuning). It adds the ground slam, the charge (wind-up, run, crash, dazed) and the rage glow on top of Zombie's chase, with poses for the new states. Its armour is data: `partMult` and `blast` on its type, read by `game.hitscan` / `game.explode`.
  - `stalker.js`: the Stalker, a `Zombie` subclass (step out of cover, watch, slip away, dash, house run, charge, lunge, hold, slash, flee, vanish, and its poses), and `StalkerDirector` (`game.stalker`), which decides when it shows up and plans each appearance from nav distance fields and lines of sight from the camera (one plan a frame; a boarded-up window counts as hiding it: `level.js` tags those colliders `boardedWindow`). `STALKER` holds the tuning. The player's side of the hold is `player.heldBy`.
  ...
  - `teammate.js`: the fireteam bots, including rifle aiming, two-bone arm IK and team lineups. Their body animation is procedural: the legs step the way they actually move relative to their facing (forward, backpedalling, sidestepping without crossing), the feet shuffle when they turn on the spot, an idle stance that breathes, shifts its weight and looks round, a lower braced stance when aiming, the rifle carried across the chest at a run, a reload (the gun cants, the support hand goes to the magwell, the hip pouch and back; shell by shell for the shotgun), a flinch when hit and a death where the knees give first.
...
- `src/player`
  ...
  - `gltfGuns.js`: GLB weapons that follow the Blender contract (see Assets). Blender-built guns get the procedural materials plus baked wear and AO; imported guns keep their textures. A gun whose support hand sits right under the eye when aiming (the P90) sets `adsLeft`: the viewmodel blends the left shoulder and elbow toward it with ADS, so the arm comes up from below instead of across the view.
...
GLB sources live in `assets/source/` and are not served (nor committed: the original downloads and intermediate `*_raw.glb` exports add up to ~400 MB; the optimized files in `public/models/` are). `node tools/optimize-assets.mjs [filter]` turns them into web-ready files in `public/models/`, using glTF-Transform (a dev dependency). It converts spec/gloss materials to metal/rough, resizes textures and re-encodes them as WebP, prunes unused data (keeping marker empties) and applies meshopt compression to everything except the skinned arms. An entry with `simplify` also cuts the triangle count to that share (the Stalker's 197k triangles to 89k). The script runs the glTF-Transform CLI through node, so it works on Windows too. `src/core/assetList.js` lists every model; they are preloaded during loading.

UniRig auto-rigs tend to bleed hand and forearm weights onto the thighs, and sometimes hip weights onto the hands, wherever the two touch in the rest pose. `node tools/fix-viper-weights.mjs --model <name> [--dry] [--out file]` repairs that. It tells arm from leg by surface distance from the hands and the knees, strips the wrong influences and blends them back in from the neighbouring vertices. It keeps the untouched file as `<name>_orig.glb`. It has been applied to Viper, the Smoker and Crusher bodies, the gas-mask zombie, the zombie woman, the Biter, the Striker, the Stalker and Nadja. The walk runs per hand, so a hand modeled as its own mesh island (Nadja's left) is reached through its own sleeve.

Models that come without a skeleton (the Worker, the plain zombie and the Survivalist, from Meshy) are rigged headless in Blender with `tools/blender/autorig.py`:

```
blender -b --factory-startup --python tools/blender/autorig.py -- in.glb out.glb [--tris 30000] [--debug]
```

It joins and welds the meshes, measures the body on the full-resolution mesh in horizontal slices (crotch, knees, ankles, toes, the narrowest slice of the neck, the chest width where arms and torso still separate, the arms followed outward from the shoulders, and a mirrored arm when one touches the body), decimates to `--tris`, builds a Mixamo-named skeleton at those landmarks, skins it with Blender's automatic (bone heat) weights and gives any vertex that got none its nearest bone. `--debug` prints the landmarks. The output then goes through `optimize-assets.mjs` like any other model (and the weight fix above if a hand touches a thigh).
...
The L4D2, Titanfall 2, SPAS-12 and Meshy models were supplied by the owner, the Stalker, Boomer (Meshy "Fat Zombie"), Worker, Survivalist and plain zombie models by a collaborator. The L4D2 Boomer is no longer used. They are fine for a private project, but check their licences before publishing the game.
