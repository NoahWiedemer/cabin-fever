...
- **Infected**
  ...
  - **Stalker**: a gaunt, pale mutant, taller than you. It haunts you from round 3 (4 on easy) in most rounds, and it's there by its second round at the latest. It isn't part of the wave: it doesn't hold up the end of a round, the INFECTED count and the radar leave it out, and it only ever goes for you (the bots don't see it until it attacks).
    - It knows every way into and through the farmhouse and the barn (only the hayloft ladder is out of its reach). It appears and vanishes only where you can't see it: behind a wall, round a corner or outside your view.
    - Every 35 to 75 s it shows up, or now and then it doesn't. It **watches**: it stands where you'll see it once you turn round (preferably under a lamp) and stares, breathing. Once you've looked at it for a moment, or come close, it backs out of sight or bolts. Or it **dashes**: it sprints across your view at 9 to 11 m/s, out of cover, past you and into cover again.
    - After two scares it **attacks** (never twice within 70 s): it bursts out of cover in front of you, screams and charges, and lunges at your face. Either it holds you for a second (you can't move, your view is dragged onto its face, you can shoot) and claws you, or it slashes you in passing. Then it runs. It comes at you from the front and circles round if you turn away. It holds you at most once per attack, so there's no stun lock.
    ...
    - `__game.stalker.force('watch' | 'dash' | 'attack')` brings it now, for testing (in a fight round).
- **How the horde hunts**
...
- `src/actors`
  ...
  - `stalker.js`: the Stalker, a `Zombie` subclass (watch, slip away, dash, charge, lunge, hold, slash, flee, vanish, and its poses), and `StalkerDirector` (`game.stalker`), which decides when it shows up and plans each appearance from nav distance fields and lines of sight from the camera. `STALKER` holds the tuning. The player's side of the hold is `player.heldBy`.
